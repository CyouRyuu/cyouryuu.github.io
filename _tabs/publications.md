...
- <span style="color:orange;">Optimization of transport costs and CO<sub>2</sub> emissions through modal shift to railways using actual shipment amount</span> (in Japaneses)<br>
Hitoshi Hara, **Aolong Zha**, Naoto Imura<br>
_Journal of Japan Logistics Society_, 2024
&nbsp;<button class="bib-button" onclick="toggleBib('Hara_Opt', this)"><span class="math">$\\BibTeX$</span></button>
<div id="Hara_Opt" class="bibtex"></div>
...
- <span style="color:orange;">Assessment of the effects of extending delivery deadline and shelf-life of processed food products on the amount of food loss by a Monte Carlo simulation</span> (in Japaneses)<br>
Hitoshi Hara, **Aolong Zha**, Naoto Imura<br>
_Journal of Japan Logistics Society_, 2024
&nbsp;<button class="bib-button" onclick="toggleBib('Hara_Asses', this)"><span class="math">$\\BibTeX$</span></button>
<div id="Hara_Asses" class="bibtex"></div>
...
- <span style="color:orange">A GPU accelerator for domain transformation-based stereo matching</span><br>
...
&nbsp;<button class="bib-button" onclick="toggleBib('Chang_A', this)"><span class="math">$\\BibTeX$</span></button>
<div id="Chang_A" class="bibtex"></div>

- <span style="color:orange;">A hybrid encoding of pseudo-Boolean constraints into CNF</span><br>
**Aolong Zha**, Miyuki Koshimura, Hiroshi Fujita<br>
_TAAI 2017: 22nd Conference on Technologies and Applications of Artificial Intelligence_
&nbsp;<button class="bib-button" onclick="toggleBib('Zha_A_hyb', this)"><span class="math">$\\BibTeX$</span></button>
<div id="Zha_A_hyb" class="bibtex"></div>
...
<style>
:root {
  --url-color: #1e90ff; /* 自定义主色（DodgerBlue） */
}
/* ========= 按钮样式 ========= */
.bib-button {
  ...
  font-size: 0.7em;
  cursor: pointer;
  transition: background-color 0.2s ease, color 0.2s ease, text-shadow 0.25s ease;
}
.bib-button:hover,
.bib-button.active {
  background-color: var(--btn-hover-bg);
  color: var(--url-color);
  text-shadow: 0 0 6px var(--url-color), 0 0 10px var(--url-color);
}
...
.bibtex {
  ...
  border-left: solid var(--url-color);
  background: var(--blockquote-bg, var(--card-bg));
...
/* BibTeX 中 DOI 链接的样式：无下划线，hover 发光 */
.bibtex a.doi-link {
...
.bibtex a.doi-link:hover,
.bibtex a.doi-link:focus-visible {
  color: var(--url-color) !important;
  text-shadow: 0 0 6px var(--url-color), 0 0 10px var(--url-color);
  outline: none;
}
...
/* ========= Dark Mode ========= */
html[data-mode="dark"] .bibtex {
  border-left-color: var(--blockquote-border-color-dark, var(--blockquote-border-color));
  background: var(--blockquote-bg-dark, var(--card-bg));
}
</style>
